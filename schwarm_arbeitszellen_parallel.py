"""
KKI Parallele Arbeitszellen-Studie
==================================
Vergleicht parallele Workflow-Zellen und koordinierte Ressourcenteilung.
"""
...
def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_PARALLEL_CELLS_REPETITIONS', '2'))
        ...
        repetitions = int(os.getenv('KKI_PARALLEL_CELLS_REPETITIONS', '3'))
    ...
    return {
        ...
        'agent_count': int(os.getenv('KKI_PARALLEL_CELLS_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
...
def profile():
    ...
    resource_budget = float(os.getenv('KKI_RESOURCE_BUDGET', '1.0'))
    resource_share = float(os.getenv('KKI_RESOURCE_SHARE_FACTOR', '0.20'))
    common = {
        **basis,
    ...
    return [
        ...
        },
        {
            ...
            'label': 'Zellen mit Ressourcen',
            **common,
            'enable_resource_coordination': True,
            'resource_budget': resource_budget,
            'resource_share_factor': resource_share,
            'enable_parallel_workflow_cells': False,
        },
        {
            'name': 'parallel-resources',
            'label': 'Parallele Zellen',
            **common,
            'enable_resource_coordination': True,
            'resource_budget': resource_budget,
            'resource_share_factor': resource_share,
    ...
    return {
        ...
        'combined_score': mittelwert(
            [
                ...
                + 0.05 * pol['workflow_metrics']['resource_share_rate']
                for pol, con in runs
            ]
        ),
        ...
        'resource_shared': mittelwert(
            [run['workflow_metrics'].get('resource_shared_total', 0.0) for run in polarization_runs]
        ),
        'resource_share_rate': mittelwert(
            [run['workflow_metrics'].get('resource_share_rate', 0.0) for run in polarization_runs]
        ),
        'active_cells': mittelwert(
            [run['workflow_metrics'].get('active_cells_mean', 0.0) for run in polarization_runs]
        ),
    }
...
def main():
    ...
    print("=" * 84)
    print("KKI PARALLELE ARBEITSZELLEN-STUDIE")
    print("=" * 84)
    print(
    ...
    for index, item in enumerate(items):
        ...
        print(
            ...
            f"Res={summary['resource_share_rate']:.2f}"
        )
    ...
    print(
        ...
        f"  Ressourcen gesamt:        {best_profile['resource_shared']:.2f}\n"
        f"  Ressourcen/Runde:         {best_profile['resource_share_rate']:.2f}\n"
        f"  Aktive Zellen/Runde:      {best_profile['active_cells']:.2f}\n"
        f"  Cross-Group-Kooperation:  {best_profile['cross_group_cooperation']:.1%}"
    )
    print(
        "\nInterpretation:\n"
        "Die Studie prueft, ob mehrere parallele Arbeitszellen durch koordinierte "
        "Ressourcenteilung ueber lineare Uebergaben hinaus stabil zusammenarbeiten."
    )
    ...
    fig.suptitle(
        'KKI Parallele Arbeitszellen: Ressourcenteilung im Schwarm',
        fontsize=14,
        fontweight='bold',
    )
    ...
    axes[1, 0].bar(
        x - width / 2,
        [summary['handoff_rate'] for summary in summaries],
        width,
    ...
    axes[1, 0].bar(
        x + width / 2,
        [summary['resource_share_rate'] for summary in summaries],
        width,
        label='Ressourcen/Runde',
        color='#64B5CD',
    )
    axes[1, 0].set_title('Uebergaben und Ressourcen')
    ...
    axes[1, 1].bar(
        x - width / 2,
        [summary['active_cells'] for summary in summaries],
        width,
        label='Aktive Zellen/Runde',
        color='#DD8452',
    )
    axes[1, 1].bar(
        x + width / 2,
    ...
    axes[1, 1].set_title('Parallele Zellen und Kooperation')
    ...
    save_and_maybe_show(plt, 'kki_arbeitszellen_parallel.png', dpi=150)
