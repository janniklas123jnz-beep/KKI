"""
KKI Engpassmanagement-Studie
===========================
Vergleicht Bottleneck-Management und prioritaetsbasierte Ressourcen-Umlenkung.
"""
...
def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_BOTTLENECK_REPETITIONS', '2'))
        ...
        repetitions = int(os.getenv('KKI_BOTTLENECK_REPETITIONS', '3'))
    ...
    return {
        ...
        'agent_count': int(os.getenv('KKI_BOTTLENECK_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
    ...
    return {
        ...
        'combined_score': mittelwert(
            [
                ...
                - 0.04 * pol['workflow_metrics']['bottleneck_rate']
                for pol, con in runs
            ]
        ),
        ...
        'bottleneck_rate': mittelwert(
            [run['workflow_metrics'].get('bottleneck_rate', 0.0) for run in polarization_runs]
        ),
        'bottleneck_relief': mittelwert(
            [run['workflow_metrics'].get('bottleneck_relief_total', 0.0) for run in polarization_runs]
        ),
        'bottleneck_relief_rate': mittelwert(
            [run['workflow_metrics'].get('bottleneck_relief_rate', 0.0) for run in polarization_runs]
        ),
        'rerouted_budget': mittelwert(
            [run['workflow_metrics'].get('rerouted_budget_total', 0.0) for run in polarization_runs]
        ),
    }
...
def main():
    ...
    print("=" * 84)
    print("KKI ENGPASSMANAGEMENT-STUDIE")
    print("=" * 84)
    print(
    ...
    for index, item in enumerate(items):
        ...
        print(
            ...
            f"BN={summary['bottleneck_rate']:.2f}, "
    ...
    print(
        ...
        f"  Bottlenecks/Runde:        {best_profile['bottleneck_rate']:.2f}\n"
        f"  Entlastung gesamt:        {best_profile['bottleneck_relief']:.2f}\n"
        f"  Umgelenktes Budget:       {best_profile['rerouted_budget']:.2f}\n"
        f"  Cross-Group-Kooperation:  {best_profile['cross_group_cooperation']:.1%}"
    )
    print(
        "\nInterpretation:\n"
        "Die Studie prueft, ob adaptive Ressourcen-Triage lokale Engpaesse zwischen "
        "heterogenen Zellclustern besser abfedert als statische Allokation."
    )
    ...
    fig.suptitle(
        'KKI Engpassmanagement: Ressourcen-Triage zwischen Zellclustern',
        fontsize=14,
        fontweight='bold',
    )
    ...
    axes[1, 0].bar(
        x - width / 2,
        [summary['bottleneck_rate'] for summary in summaries],
        width,
        label='Bottlenecks/Runde',
        color='#8172B2',
    )
    axes[1, 0].bar(
        x + width / 2,
    ...
    axes[1, 0].set_title('Engpassdruck und Entlastung')
    ...
    axes[1, 1].bar(
        x - width / 2,
        [summary['rerouted_budget'] for summary in summaries],
        width,
        label='Umgelenktes Budget',
        color='#DD8452',
    )
    axes[1, 1].bar(
        x + width / 2,
    ...
    axes[1, 1].set_title('Triage und Kooperation')
    ...
    save_and_maybe_show(plt, 'kki_engpassmanagement.png', dpi=150)
