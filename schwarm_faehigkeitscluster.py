"""
KKI Faehigkeitscluster-Studie
=============================
Vergleicht heterogene Zellcluster mit asymmetrischen Ressourcenbudgets.
"""
...
def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_CAPABILITY_CLUSTER_REPETITIONS', '2'))
        ...
        repetitions = int(os.getenv('KKI_CAPABILITY_CLUSTER_REPETITIONS', '3'))
    ...
    return {
        ...
        'agent_count': int(os.getenv('KKI_CAPABILITY_CLUSTER_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
...
def profile():
    ...
    common = {
        **basis,
        ...
        'cluster_budget_skew': float(os.getenv('KKI_CLUSTER_BUDGET_SKEW', '0.35')),
    }
    return [
    ...
    return {
        ...
        'combined_score': mittelwert(
            [
                ...
                + 0.05 * pol['workflow_metrics']['cluster_output_rate']
                for pol, con in runs
            ]
        ),
        ...
        'cluster_output': mittelwert(
            [run['workflow_metrics'].get('cluster_output_total', 0.0) for run in polarization_runs]
        ),
        'cluster_output_rate': mittelwert(
            [run['workflow_metrics'].get('cluster_output_rate', 0.0) for run in polarization_runs]
        ),
...
def main():
    ...
    print("=" * 84)
    print("KKI FAEHIGKEITSCLUSTER-STUDIE")
    print("=" * 84)
    print(
    ...
    for index, item in enumerate(items):
        ...
        print(
            ...
            f"Eff={summary['resource_efficiency']:.2f}, "
            f"Output={summary['cluster_output_rate']:.2f}"
        )
    ...
    print(
        ...
        f"  Cluster-Output gesamt:    {best_profile['cluster_output']:.2f}\n"
        f"  Cluster-Output/Runde:     {best_profile['cluster_output_rate']:.2f}\n"
        f"  Aktive Zellen/Runde:      {best_profile['active_cells']:.2f}\n"
        f"  Cross-Group-Kooperation:  {best_profile['cross_group_cooperation']:.1%}"
    )
    print(
        "\nInterpretation:\n"
        "Die Studie prueft, ob heterogene Clusterprofile und asymmetrische Budgets "
        "die parallele Zellkoordination robuster und produktiver machen."
    )
    ...
    fig.suptitle(
        'KKI Faehigkeitscluster: Heterogene Zellprofile und Ressourcenbudgets',
        fontsize=14,
        fontweight='bold',
    )
    ...
    axes[1, 0].bar(
        x + width / 2,
        [summary['cluster_output_rate'] for summary in summaries],
        width,
        label='Cluster-Output/Runde',
        color='#64B5CD',
    )
    axes[1, 0].set_title('Output und Effizienz')
    ...
    axes[1, 1].set_title('Aktive Zellen und Kooperation')
    ...
    save_and_maybe_show(plt, 'kki_faehigkeitscluster.png', dpi=150)
