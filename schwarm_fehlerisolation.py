"""
KKI Fehlerisolations-Studie
===========================
Vergleicht Quarantaene und Vertrauensabschirmung fuer kompromittierte Teilzellen
unter Promptinjektion und strategischer Fehlinformation.
"""
...
def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_ISOLATION_REPETITIONS', '2'))
        ...
        repetitions = int(os.getenv('KKI_ISOLATION_REPETITIONS', '3'))
    ...
    return {
        ...
        'agent_count': int(os.getenv('KKI_ISOLATION_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
...
def profile(params):
    ...
    common = {
        **basis,
        ...
        'resource_budget': float(os.getenv('KKI_ISOLATION_RESOURCE_BUDGET', '0.55')),
    ...
    return [
        ...
        {
            'name': 'emergent-under-attack',
            'label': 'Emergente Skills unter Angriff',
            **common,
        ...
        {
            'name': 'cell-quarantine',
            'label': 'Zellquarantaene',
            **common,
            'enable_emergent_skills': True,
            'group_learning_rate': float(os.getenv('KKI_GROUP_LEARNING_RATE', '0.12')),
            'skill_specialization_threshold': float(os.getenv('KKI_SKILL_SPECIALIZATION_THRESHOLD', '0.60')),
            'enable_fault_isolation': True,
            'quarantine_compromise_threshold': float(os.getenv('KKI_QUARANTINE_COMPROMISE_THRESHOLD', '0.26')),
            'quarantine_exposure_threshold': float(os.getenv('KKI_QUARANTINE_EXPOSURE_THRESHOLD', '0.32')),
            'trust_shield_strength': float(os.getenv('KKI_TRUST_SHIELD_STRENGTH', '0.18')),
        },
        {
            'name': 'shielded-quarantine',
            'label': 'Abgeschirmte Quarantaene',
            **common,
            'enable_emergent_skills': True,
            'group_learning_rate': float(os.getenv('KKI_GROUP_LEARNING_RATE', '0.12')),
            ...
            'trust_shield_strength': float(os.getenv('KKI_TRUST_SHIELD_STRENGTH', '0.28')),
            'sentinel_share': 0.20,
            'analyzer_share': 0.20,
    ...
    return {
        ...
        'combined_score': mittelwert(
            [
                ...
                + 0.14 * pol['workflow_metrics'].get('resource_efficiency', 0.0)
                + 0.10 * pol['workflow_metrics'].get('misinformation_detection_rate', 0.0)
                + 0.08 * pol['workflow_metrics'].get('skill_alignment_rate', 0.0)
                + 0.08 * pol['workflow_metrics'].get('trust_shield_mean', 0.0)
                + 0.06 * pol['workflow_metrics'].get('isolation_relief_rate', 0.0)
                - 0.08 * pol['workflow_metrics'].get('misinformation_corruption_mean', 0.0)
                - 0.08 * pol['workflow_metrics'].get('cluster_compromise_mean', 0.0)
                - 0.03 * pol['workflow_metrics'].get('quarantined_agents_mean', 0.0)
                for pol, con in runs
            ]
        ),
        ...
        'detection_rate': mittelwert([run['workflow_metrics'].get('misinformation_detection_rate', 0.0) for run in polarization_runs]),
        'corruption_mean': mittelwert([run['workflow_metrics'].get('misinformation_corruption_mean', 0.0) for run in polarization_runs]),
        'skill_alignment_rate': mittelwert([run['workflow_metrics'].get('skill_alignment_rate', 0.0) for run in polarization_runs]),
        'quarantined_agents_mean': mittelwert([run['workflow_metrics'].get('quarantined_agents_mean', 0.0) for run in polarization_runs]),
        'trust_shield_mean': mittelwert([run['workflow_metrics'].get('trust_shield_mean', 0.0) for run in polarization_runs]),
        'isolation_relief_rate': mittelwert([run['workflow_metrics'].get('isolation_relief_rate', 0.0) for run in polarization_runs]),
    }
...
def main():
    ...
    print('=' * 84)
    print('KKI FEHLERISOLATIONS-STUDIE')
    print('=' * 84)
    print('Untersucht, ob Quarantaene und Vertrauensabschirmung kompromittierte Teilzellen')
    print('unter Angriff isolieren, ohne die kooperative Schwarmleistung zu zerstoeren.\n')
    ...
    for index, item in enumerate(items):
        ...
        print(
            ...
            f"Shield={summary['trust_shield_mean']:.2f} | "
            f"Quar={summary['quarantined_agents_mean']:.2f}"
        )

    best = max(summaries, key=lambda item: item['combined_score'])
    print('\nBeste Isolationsarchitektur:', best['label'])
    ...
    print(
        ...
        f"Abschirmniveau {best['trust_shield_mean']:.2f}, "
        f"Ø quarantinierte Agenten {best['quarantined_agents_mean']:.2f}"
    )
    ...
    corruption = [item['corruption_mean'] for item in summaries]
    shield = [item['trust_shield_mean'] for item in summaries]
    quarantine = [item['quarantined_agents_mean'] for item in summaries]

    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    ax1, ax2, ax3, ax4 = axes.flat

    colors = ['#9aa0a6', '#6c8ebf', '#59a14f', '#f28e2b', '#e15759']
    ...
    ax3.bar(labels, corruption, color=colors)
    ax3.set_title('Mittlere Korruption')
    ax3.tick_params(axis='x', rotation=18)

    ax4.bar(labels, shield, color=colors, label='Abschirmniveau')
    ax4.plot(labels, quarantine, color='#222222', marker='o', linewidth=2, label='Ø quarant. Agenten')
    ax4.set_title('Quarantaene und Abschirmung')
    ax4.tick_params(axis='x', rotation=18)
    ax4.legend()

    fig.tight_layout()
    save_and_maybe_show(plt, 'kki_fehlerisolation.png', dpi=150)
