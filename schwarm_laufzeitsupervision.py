"""
KKI Laufzeitsupervisions-Studie
===============================
Vergleicht Supervisor- und Watchdog-Strukturen fuer laufende Gruppenprozesse
und misst, welche Aufsicht Drift, Ueberlastung und Fehlverhalten frueh erkennt,
gezielt eingreift und den Baufluss dennoch stabil haelt.
"""

from __future__ import annotations

import os
import random

import matplotlib.pyplot as plt
import numpy as np

from kki_runtime import DEFAULT_SEED, configure_matplotlib, is_test_mode, save_and_maybe_show
from schwarm_dna_schema import get_named, schema_metrics
from schwarm_governance_layer import governance_profiles
from schwarm_gruppenbildung import gruppenmetriken
from schwarm_gruppenidentitaet import modellkatalog, mittelwert
from schwarm_interaktionsmodelle import base_config as interaction_base_config
from schwarm_polarisierung import (
    DEFAULT_AGENT_COUNT,
    DEFAULT_CONNECTIONS_PER_AGENT,
    DEFAULT_INTERACTIONS_PER_ROUND,
    DEFAULT_ROUNDS,
    run_polarization_experiment,
)
from schwarm_werkzeugbindung import tool_binding_profiles


def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_RUNTIME_SUPERVISION_REPETITIONS', '2'))
        rounds = min(DEFAULT_ROUNDS, int(os.getenv('KKI_TEST_ROUNDS', '6')))
        interactions = min(DEFAULT_INTERACTIONS_PER_ROUND, int(os.getenv('KKI_TEST_INTERACTIONS', '8')))
        attack_round = min(max(2, int(os.getenv('KKI_TEST_INVASION_ROUND', '3'))), rounds - 2)
        failure_round = min(rounds - 1, attack_round + 1)
    else:
        repetitions = int(os.getenv('KKI_RUNTIME_SUPERVISION_REPETITIONS', '3'))
        rounds = DEFAULT_ROUNDS
        interactions = DEFAULT_INTERACTIONS_PER_ROUND
        attack_round = int(os.getenv('KKI_INJECTION_ATTACK_ROUND', '90'))
        failure_round = int(os.getenv('KKI_FAILURE_ROUND', '110'))

    return {
        'repetitions': repetitions,
        'rounds': rounds,
        'interactions': interactions,
        'attack_round': attack_round,
        'failure_round': min(rounds - 1, max(attack_round + 1, failure_round)),
        'agent_count': int(os.getenv('KKI_RUNTIME_SUPERVISION_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
        'degree': int(os.getenv('KKI_NETWORK_DEGREE', str(DEFAULT_CONNECTIONS_PER_AGENT))),
        'stress_strength': float(os.getenv('KKI_RUNTIME_SUPERVISION_STRESS_STRENGTH', '0.48')),
        'stress_sources': int(os.getenv('KKI_RUNTIME_SUPERVISION_STRESS_SOURCES', '8')),
        'failure_duration': int(os.getenv('KKI_FAILURE_DURATION', '8')),
        'failure_fraction': float(os.getenv('KKI_RUNTIME_SUPERVISION_FAILURE_FRACTION', '0.18')),
        'resync_strength': float(os.getenv('KKI_RESYNC_STRENGTH', '0.36')),
        'bond_history_threshold': int(os.getenv('KKI_GROUP_BOND_HISTORY_THRESHOLD', '18')),
        'bond_trust_threshold': float(os.getenv('KKI_GROUP_BOND_TRUST_THRESHOLD', '0.76')),
        'proto_group_min_size': int(os.getenv('KKI_PROTO_GROUP_MIN_SIZE', '4')),
        'functional_group_min_size': int(os.getenv('KKI_FUNCTIONAL_GROUP_MIN_SIZE', '5')),
        'functional_alignment_threshold': float(os.getenv('KKI_FUNCTIONAL_ALIGNMENT_THRESHOLD', '0.60')),
        'group_opinion_distance_threshold': float(os.getenv('KKI_GROUP_OPINION_DISTANCE_THRESHOLD', '0.18')),
        'top_relationships_per_agent': int(os.getenv('KKI_GROUP_TOP_RELATIONSHIPS', '3')),
    }


def clamp01(value):
    return max(0.0, min(1.0, float(value)))


def normalize(value, minimum, maximum):
    if maximum <= minimum:
        return 0.0
    return clamp01((float(value) - minimum) / (maximum - minimum))


def kontexte():
    return [
        {'name': 'startup', 'label': 'Startup', 'scenario': 'consensus', 'startup_mode': True},
        {'name': 'runtime', 'label': 'Runtime', 'scenario': 'consensus'},
        {
            'name': 'stress',
            'label': 'Manipulationsstress',
            'scenario': 'polarization',
            'enable_prompt_injection': True,
        },
        {
            'name': 'recovery',
            'label': 'Recovery',
            'scenario': 'polarization',
            'enable_prompt_injection': True,
            'enable_failures': True,
        },
    ]


def supervision_profiles(katalog, params):
    governance = get_named(governance_profiles(katalog, params), 'resilienter-governance-layer')['config']
    tool_binding = get_named(tool_binding_profiles(katalog, params), 'resiliente-werkzeugfoederation')['base_config']

    return [
        {
            'name': 'selbstaufsicht',
            'label': 'Selbstaufsicht',
            'config': {
                **tool_binding,
                'enable_meta_coordination': False,
                'mission_arbitration_enabled': False,
                'enable_fault_isolation': False,
                'enable_bottleneck_management': False,
                'meta_update_interval': 8,
                'mission_switch_interval': 6,
                'resource_share_factor': 0.24,
                'trust_shield_strength': 0.22,
            },
            'bias': {'watch': 0.18, 'drift': 0.22, 'overload': 0.18, 'intervention': 0.16, 'recovery': 0.22, 'overhead': 0.08},
        },
        {
            'name': 'metriken-monitor',
            'label': 'Metriken-Monitor',
            'config': {
                **tool_binding,
                'enable_meta_coordination': True,
                'mission_arbitration_enabled': False,
                'enable_fault_isolation': False,
                'enable_bottleneck_management': True,
                'meta_update_interval': 6,
                'bottleneck_threshold': 0.96,
                'bottleneck_triage_intensity': 0.84,
                'meta_priority_strength': 0.34,
                'trust_shield_strength': 0.28,
            },
            'bias': {'watch': 0.52, 'drift': 0.50, 'overload': 0.54, 'intervention': 0.36, 'recovery': 0.40, 'overhead': 0.20},
        },
        {
            'name': 'watchdog-ring',
            'label': 'Watchdog-Ring',
            'config': {
                **governance,
                **tool_binding,
                'enable_meta_coordination': True,
                'mission_arbitration_enabled': True,
                'enable_fault_isolation': True,
                'enable_bottleneck_management': True,
                'meta_update_interval': 5,
                'bottleneck_threshold': 0.94,
                'bottleneck_triage_intensity': 0.88,
                'trust_shield_strength': 0.34,
                'resource_share_factor': 0.28,
            },
            'bias': {'watch': 0.76, 'drift': 0.78, 'overload': 0.70, 'intervention': 0.72, 'recovery': 0.62, 'overhead': 0.28},
        },
        {
            'name': 'triage-supervisor',
            'label': 'Triage-Supervisor',
            'config': {
                **governance,
                **tool_binding,
                'enable_meta_coordination': True,
                'mission_arbitration_enabled': True,
                'enable_fault_isolation': True,
                'enable_bottleneck_management': True,
                'meta_update_interval': 4,
                'bottleneck_threshold': 0.92,
                'bottleneck_triage_intensity': 0.94,
                'trust_shield_strength': 0.36,
                'resource_share_factor': 0.29,
                'cluster_budget_skew': 0.18,
            },
            'bias': {'watch': 0.84, 'drift': 0.80, 'overload': 0.86, 'intervention': 0.84, 'recovery': 0.74, 'overhead': 0.32},
        },
        {
            'name': 'recovery-supervisor',
            'label': 'Recovery-Supervisor',
            'config': {
                **governance,
                **tool_binding,
                'enable_meta_coordination': True,
                'mission_arbitration_enabled': True,
                'enable_fault_isolation': True,
                'enable_bottleneck_management': True,
                'meta_update_interval': 4,
                'bottleneck_threshold': 0.94,
                'bottleneck_triage_intensity': 0.90,
                'trust_shield_strength': 0.38,
                'resync_strength': min(0.54, governance.get('resync_strength', params['resync_strength']) + 0.06),
                'failure_fraction': max(0.10, params['failure_fraction'] - 0.04),
            },
            'bias': {'watch': 0.78, 'drift': 0.76, 'overload': 0.72, 'intervention': 0.80, 'recovery': 0.90, 'overhead': 0.30},
        },
        {
            'name': 'resiliente-supervisor-foederation',
            'label': 'Resiliente Supervisor-Foederation',
            'config': {
                **governance,
                **tool_binding,
                'enable_meta_coordination': True,
                'mission_arbitration_enabled': True,
                'enable_fault_isolation': True,
                'enable_bottleneck_management': True,
                'meta_update_interval': 4,
                'meta_priority_strength': 0.42,
                'bottleneck_threshold': 0.92,
                'bottleneck_triage_intensity': 0.96,
                'trust_shield_strength': 0.40,
                'resource_budget': 0.80,
                'resource_share_factor': 0.30,
                'cluster_budget_skew': 0.16,
                'resync_strength': min(0.54, governance.get('resync_strength', params['resync_strength']) + 0.08),
                'failure_fraction': max(0.10, params['failure_fraction'] - 0.05),
            },
            'bias': {'watch': 0.92, 'drift': 0.90, 'overload': 0.90, 'intervention': 0.92, 'recovery': 0.92, 'overhead': 0.30},
        },
    ]


def supervision_metrics(config, result, params, bias):
    workflow = result['workflow_metrics']
    groups = gruppenmetriken(result['agents'], params)
    schema = schema_metrics(config)
    agent_count = max(1.0, params['agent_count'])

    recoveries = workflow.get('recovery_events_total', 0.0) / agent_count
    failed_share = workflow.get('failed_agents_mean', 0.0) / agent_count
    arbitration_activity = normalize(result.get('mission_arbitration_total', 0.0), 0.0, 40.0)
    bottleneck_rate = workflow.get('bottleneck_rate', 0.0)
    bottleneck_relief_rate = workflow.get('bottleneck_relief_rate', 0.0)

    watch_coverage = clamp01(
        mittelwert(
            [
                1.0 if config.get('enable_meta_coordination', False) else 0.0,
                1.0 if config.get('enable_bottleneck_management', False) else 0.0,
                normalize(config.get('meta_update_interval', 8), 8.0, 4.0),
                workflow.get('meta_alignment_rate', 0.0),
                bias['watch'],
            ]
        )
    )
    drift_detection = clamp01(
        mittelwert(
            [
                workflow.get('misinformation_detection_rate', 0.0),
                workflow.get('trust_shield_mean', 0.0),
                1.0 - workflow.get('misinformation_corruption_mean', 0.0),
                1.0 - workflow.get('cluster_compromise_mean', 0.0),
                bias['drift'],
            ]
        )
    )
    overload_relief = clamp01(
        mittelwert(
            [
                1.0 - bottleneck_rate,
                bottleneck_relief_rate,
                workflow.get('resource_efficiency', 0.0),
                workflow.get('resource_share_rate', 0.0),
                bias['overload'],
            ]
        )
    )
    intervention_quality = clamp01(
        mittelwert(
            [
                1.0 if config.get('mission_arbitration_enabled', False) else 0.0,
                1.0 if config.get('enable_fault_isolation', False) else 0.0,
                arbitration_activity,
                workflow.get('handoff_rate', 0.0),
                workflow.get('completion_rate', 0.0),
                bias['intervention'],
            ]
        )
    )
    recovery_coordination = clamp01(
        mittelwert(
            [
                recoveries,
                workflow.get('sync_strength_mean', 0.0),
                1.0 - failed_share,
                normalize(config.get('resync_strength', params['resync_strength']), 0.24, 0.54),
                bias['recovery'],
            ]
        )
    )
    supervision_stability = clamp01(
        mittelwert(
            [
                schema['safety_margin'],
                groups['functional_group_share'],
                result['cross_group_cooperation_rate'],
                workflow.get('skill_alignment_rate', 0.0),
                1.0 - groups['group_separation'],
            ]
        )
    )
    supervision_overhead = clamp01(
        mittelwert(
            [
                normalize(config.get('meta_update_interval', 4), 4.0, 8.0),
                arbitration_activity,
                normalize(config.get('resource_share_factor', 0.24), 0.24, 0.30),
                normalize(config.get('bottleneck_triage_intensity', 0.80), 0.80, 0.96),
                bias['overhead'],
            ]
        )
    )

    return {
        'completion_rate': workflow.get('completion_rate', 0.0),
        'resource_efficiency': workflow.get('resource_efficiency', 0.0),
        'meta_alignment_rate': workflow.get('meta_alignment_rate', 0.0),
        'skill_alignment_rate': workflow.get('skill_alignment_rate', 0.0),
        'handoff_rate': workflow.get('handoff_rate', 0.0),
        'resource_share_rate': workflow.get('resource_share_rate', 0.0),
        'detection_rate': workflow.get('misinformation_detection_rate', 0.0),
        'corruption_mean': workflow.get('misinformation_corruption_mean', 0.0),
        'cluster_compromise_mean': workflow.get('cluster_compromise_mean', 0.0),
        'trust_shield_mean': workflow.get('trust_shield_mean', 0.0),
        'quarantined_agents_mean': workflow.get('quarantined_agents_mean', 0.0),
        'recovery_events_total': workflow.get('recovery_events_total', 0.0),
        'sync_strength_mean': workflow.get('sync_strength_mean', 0.0),
        'failed_agents_mean': workflow.get('failed_agents_mean', 0.0),
        'bottleneck_rate': bottleneck_rate,
        'bottleneck_relief_rate': bottleneck_relief_rate,
        'mission_success': mittelwert(list(result.get('mission_success_rates', {}).values())),
        'cross_group_cooperation': result['cross_group_cooperation_rate'],
        'consensus': result['final_consensus_score'],
        'polarization': result['final_polarization_index'],
        'watch_coverage': watch_coverage,
        'drift_detection': drift_detection,
        'overload_relief': overload_relief,
        'intervention_quality': intervention_quality,
        'recovery_coordination': recovery_coordination,
        'supervision_stability': supervision_stability,
        'supervision_overhead': supervision_overhead,
    }


def run_context(seed, params, profile, kontext):
    random.seed(seed)
    np.random.seed(seed)
    config = interaction_base_config(params, kontext['scenario'])
    config.update(profile['config'])
    config['agent_count'] = params['agent_count']
    config['rounds'] = params['rounds']
    config['interactions_per_round'] = params['interactions']
    config['connections_per_agent'] = params['degree']
    config['rewire_target_degree'] = params['degree']

    if kontext.get('startup_mode'):
        config['enable_prompt_injection'] = False
        config['workflow_stage_min_tenure'] = 1
    elif kontext.get('enable_prompt_injection'):
        config['enable_prompt_injection'] = True
        config['injection_attack_round'] = params['attack_round']
        config['injection_strength'] = params['stress_strength']
        config['injection_source_count'] = params['stress_sources']
    else:
        config['enable_prompt_injection'] = False

    if kontext.get('enable_failures'):
        config['enable_cluster_failures'] = True
        config['enable_restart_recovery'] = True
        config['failure_round'] = params['failure_round']
        config['failure_duration'] = params['failure_duration']
        config.setdefault('failure_fraction', params['failure_fraction'])
        config.setdefault('resync_strength', params['resync_strength'])

    result = run_polarization_experiment(config, make_plot=False, print_summary=False)
    result['supervision_metrics'] = supervision_metrics(config, result, params, profile['bias'])
    return result


def context_score(kontext_name, result, agent_count):
    metrics = result['supervision_metrics']
    base = metrics['consensus'] - metrics['polarization']
    failed_share = metrics['failed_agents_mean'] / max(1.0, agent_count)
    recoveries = metrics['recovery_events_total'] / max(1.0, agent_count)

    if kontext_name == 'startup':
        return (
            metrics['completion_rate']
            + 0.16 * metrics['watch_coverage']
            + 0.14 * metrics['intervention_quality']
            + 0.10 * metrics['supervision_stability']
            + 0.10 * metrics['drift_detection']
            - 0.12 * metrics['supervision_overhead']
        )
    if kontext_name == 'runtime':
        return (
            metrics['completion_rate']
            + 0.16 * metrics['watch_coverage']
            + 0.16 * metrics['overload_relief']
            + 0.12 * metrics['intervention_quality']
            + 0.10 * metrics['supervision_stability']
            - 0.10 * metrics['supervision_overhead']
        )
    if kontext_name == 'stress':
        return (
            base
            + 0.16 * metrics['drift_detection']
            + 0.14 * metrics['intervention_quality']
            + 0.12 * metrics['watch_coverage']
            + 0.10 * metrics['overload_relief']
            - 0.14 * metrics['corruption_mean']
            - 0.10 * metrics['cluster_compromise_mean']
            - 0.08 * metrics['supervision_overhead']
        )
    return (
        base
        + 0.18 * metrics['recovery_coordination']
        + 0.14 * recoveries
        + 0.10 * metrics['intervention_quality']
        + 0.08 * metrics['supervision_stability']
        - 0.12 * failed_share
        - 0.08 * metrics['cluster_compromise_mean']
        - 0.08 * metrics['supervision_overhead']
    )


def summarize_runs(runs, profile, context_list, agent_count):
    context_scores = {}
    metrics = {
        'watch_coverage': {},
        'drift_detection': {},
        'overload_relief': {},
        'intervention_quality': {},
        'recovery_coordination': {},
        'supervision_stability': {},
        'supervision_overhead': {},
        'detection_rate': {},
        'bottleneck_relief_rate': {},
    }

    for kontext in context_list:
        name = kontext['name']
        context_scores[name] = mittelwert([context_score(name, run[name], agent_count) for run in runs])
        for metric_name in metrics:
            metrics[metric_name][name] = mittelwert([run[name]['supervision_metrics'][metric_name] for run in runs])

    return {
        'name': profile['name'],
        'label': profile['label'],
        'combined_score': mittelwert(
            [sum(context_score(kontext['name'], run[kontext['name']], agent_count) for kontext in context_list) for run in runs]
        ),
        'context_scores': context_scores,
        **metrics,
    }


def main():
    configure_matplotlib(plt)
    params = studienparameter()
    katalog = modellkatalog(params)
    profiles = supervision_profiles(katalog, params)
    context_list = kontexte()
    base_seed = int(os.getenv('KKI_SEED', str(DEFAULT_SEED)))

    print('=' * 84)
    print('KKI LAUFZEITSUPERVISIONS-STUDIE')
    print('=' * 84)
    print('Vergleicht Supervisor- und Watchdog-Schichten fuer laufende Baugruppen,')
    print('damit Drift, Ueberlastung und Fehlverhalten frueh erkannt und stabilisiert werden.\n')

    summaries = []
    for profile_index, profile in enumerate(profiles):
        runs = []
        for repetition in range(params['repetitions']):
            repetition_seed = base_seed + profile_index * 100 + repetition
            run = {}
            for context_index, kontext in enumerate(context_list):
                run[kontext['name']] = run_context(
                    repetition_seed + context_index * 1000,
                    params,
                    profile,
                    kontext,
                )
            runs.append(run)

        summary = summarize_runs(runs, profile, context_list, params['agent_count'])
        summaries.append(summary)
        print(
            f"{summary['label']:<32} Score={summary['combined_score']:+.3f} | "
            f"Watch={summary['watch_coverage']['runtime']:.2f} | "
            f"Relief={summary['overload_relief']['runtime']:.2f} | "
            f"Recovery={summary['recovery_coordination']['recovery']:.2f}"
        )

    best = max(summaries, key=lambda item: item['combined_score'])
    baseline = next(item for item in summaries if item['name'] == 'selbstaufsicht')

    print('\nBeste Laufzeitsupervision:', best['label'])
    print(
        f"Score {best['combined_score']:+.3f} | "
        f"Delta zur Selbstaufsicht {best['combined_score'] - baseline['combined_score']:+.3f}"
    )
    print(
        f"Watch {best['watch_coverage']['runtime']:.2f}, "
        f"Intervention {best['intervention_quality']['stress']:.2f}, "
        f"Recovery {best['recovery_coordination']['recovery']:.2f}"
    )

    labels = [item['label'] for item in summaries]
    x = np.arange(len(labels))
    width = 0.35
    colors = ['#9aa0a6', '#f28e2b', '#4e79a7', '#59a14f', '#76b7b2', '#e15759']

    fig, axes = plt.subplots(2, 3, figsize=(19, 10))

    axes[0, 0].bar(x, [item['combined_score'] for item in summaries], color=colors)
    axes[0, 0].set_title('Kombinierter Supervisions-Score')
    axes[0, 0].set_xticks(x)
    axes[0, 0].set_xticklabels(labels, rotation=18)
    axes[0, 0].grid(True, axis='y', alpha=0.3)

    axes[0, 1].bar(
        x - width / 2,
        [item['watch_coverage']['runtime'] for item in summaries],
        width,
        label='Watch',
        color='#4e79a7',
    )
    axes[0, 1].bar(
        x + width / 2,
        [item['intervention_quality']['stress'] for item in summaries],
        width,
        label='Intervention',
        color='#59a14f',
    )
    axes[0, 1].set_title('Aufsicht und Eingriff')
    axes[0, 1].set_xticks(x)
    axes[0, 1].set_xticklabels(labels, rotation=18)
    axes[0, 1].legend()
    axes[0, 1].grid(True, axis='y', alpha=0.3)

    axes[0, 2].bar(
        x - width / 2,
        [item['overload_relief']['runtime'] for item in summaries],
        width,
        label='Entlastung',
        color='#76b7b2',
    )
    axes[0, 2].bar(
        x + width / 2,
        [item['supervision_overhead']['runtime'] for item in summaries],
        width,
        label='Overhead',
        color='#e15759',
    )
    axes[0, 2].set_title('Entlastung vs. Aufwand')
    axes[0, 2].set_xticks(x)
    axes[0, 2].set_xticklabels(labels, rotation=18)
    axes[0, 2].legend()
    axes[0, 2].grid(True, axis='y', alpha=0.3)

    heatmap_data = np.array(
        [
            [
                item['context_scores']['startup'],
                item['context_scores']['runtime'],
                item['context_scores']['stress'],
                item['context_scores']['recovery'],
                item['recovery_coordination']['recovery'],
            ]
            for item in summaries
        ],
        dtype=float,
    )
    heatmap = axes[1, 0].imshow(heatmap_data, cmap='viridis', aspect='auto')
    axes[1, 0].set_title('Supervisions-Kontexte')
    axes[1, 0].set_xticks([0, 1, 2, 3, 4])
    axes[1, 0].set_xticklabels(['Start', 'Run', 'Stress', 'Rec', 'Rec+'])
    axes[1, 0].set_yticks(range(len(labels)))
    axes[1, 0].set_yticklabels(labels)
    for row in range(heatmap_data.shape[0]):
        for col in range(heatmap_data.shape[1]):
            axes[1, 0].text(col, row, f'{heatmap_data[row, col]:.2f}', ha='center', va='center', fontsize=8)
    fig.colorbar(heatmap, ax=axes[1, 0], fraction=0.046, pad=0.04)

    axes[1, 1].bar(
        x - width / 2,
        [item['drift_detection']['stress'] * 100.0 for item in summaries],
        width,
        label='Drift-Detektion (%)',
        color='#edc948',
    )
    axes[1, 1].bar(
        x + width / 2,
        [item['bottleneck_relief_rate']['runtime'] * 100.0 for item in summaries],
        width,
        label='Entlastung (%)',
        color='#b07aa1',
    )
    axes[1, 1].set_title('Detektion und Runtime-Relief')
    axes[1, 1].set_xticks(x)
    axes[1, 1].set_xticklabels(labels, rotation=18)
    axes[1, 1].legend()
    axes[1, 1].grid(True, axis='y', alpha=0.3)

    axes[1, 2].axis('off')
    axes[1, 2].text(
        0.0,
        0.98,
        (
            "Zusammenfassung\n"
            f"- Bestes Profil: {best['label']}\n"
            f"- Delta zur Selbstaufsicht: {best['combined_score'] - baseline['combined_score']:+.3f}\n"
            f"- Watch: {best['watch_coverage']['runtime']:.2f}\n"
            f"- Drift-Detektion: {best['drift_detection']['stress']:.2f}\n"
            f"- Entlastung: {best['overload_relief']['runtime']:.2f}\n"
            f"- Intervention: {best['intervention_quality']['stress']:.2f}\n"
            f"- Recovery: {best['recovery_coordination']['recovery']:.2f}\n"
            f"- Overhead: {best['supervision_overhead']['runtime']:.2f}"
        ),
        va='top',
        fontsize=10,
    )

    fig.tight_layout()
    save_and_maybe_show(plt, 'kki_laufzeitsupervision.png', dpi=150)


if __name__ == '__main__':
    main()
