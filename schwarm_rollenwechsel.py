"""
KKI Rollenwechsel-Studie
========================
Vergleicht statische und dynamische Rollen auf gemeinsamer Agenten-DNA.
"""
...
def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_ROLLENWECHSEL_REPETITIONS', '2'))
        ...
        repetitions = int(os.getenv('KKI_ROLLENWECHSEL_REPETITIONS', '3'))
    ...
    return {
        ...
        'agent_count': int(os.getenv('KKI_ROLLENWECHSEL_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
...
def profile():
    ...
    return [
        ...
        {'name': 'static-mix', 'label': 'Statischer Lernmix', **learning_mix},
        {
            'name': 'dynamic-switching',
            'label': 'Dynamischer Rollenwechsel',
            **learning_mix,
...
def summarize_runs(runs, item):
    ...
    return {
        ...
        'role_switches': mittelwert([run['role_switch_total'] for run in polarization_runs]),
        'transition_diversity': mittelwert(
            [len([count for count in run['role_transition_counts'].values() if count > 0]) for run in polarization_runs]
        ),
...
def main():
    ...
    print("=" * 84)
    print("KKI ROLLENWECHSEL-STUDIE")
    print("=" * 84)
    print(
    ...
    for index, item in enumerate(items):
        ...
        print(
            ...
            f"Wechsel={summary['role_switches']:.1f}"
        )
    ...
    print(
        ...
        f"  Übergangsvielfalt:        {best_profile['transition_diversity']:.1f}\n"
        f"  Cross-Group-Kooperation:  {best_profile['cross_group_cooperation']:.1%}"
    )
    print(
        "\nInterpretation:\n"
        "Die Studie prueft, ob ein Schwarm von stabilen Spezialrollen zu einer "
        "anpassungsfaehigen Rollenverteilung uebergehen sollte, ohne seine gemeinsame DNA zu verlieren."
    )
    ...
    fig.suptitle(
        'KKI Rollenwechsel: Dynamische Aufgabenverteilung im Schwarm',
        fontsize=14,
        fontweight='bold',
    )
    ...
    axes[1, 0].bar(
        x + width / 2,
        [summary['transition_diversity'] for summary in summaries],
        width,
        label='Übergangsvielfalt',
        color='#CCB974',
    )
    axes[1, 0].set_title('Dynamik der Rollenwechsel')
    ...
    axes[1, 1].set_title('Spezialrollen-Kooperation')
    ...
    save_and_maybe_show(plt, 'kki_rollenwechsel.png', dpi=150)
