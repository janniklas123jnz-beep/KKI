"""
KKI Sandbox-Zellen-Studie
=========================
Vergleicht Zell- und Sandbox-Strukturen fuer operative Baugruppen und misst,
welche Ausfuehrungsraeume Werkzeuge, Speicher und Supervisoren sicher koppeln,
ohne Kontrolle und Arbeitsfluss zu entgrenzen.
"""
...
def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_SANDBOX_CELL_REPETITIONS', '2'))
        ...
        repetitions = int(os.getenv('KKI_SANDBOX_CELL_REPETITIONS', '3'))
    ...
    return {
        ...
        'agent_count': int(os.getenv('KKI_SANDBOX_CELL_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
        'degree': int(os.getenv('KKI_NETWORK_DEGREE', str(DEFAULT_CONNECTIONS_PER_AGENT))),
        'stress_strength': float(os.getenv('KKI_SANDBOX_CELL_STRESS_STRENGTH', '0.48')),
        'stress_sources': int(os.getenv('KKI_SANDBOX_CELL_STRESS_SOURCES', '8')),
        'failure_duration': int(os.getenv('KKI_FAILURE_DURATION', '8')),
        'failure_fraction': float(os.getenv('KKI_SANDBOX_CELL_FAILURE_FRACTION', '0.18')),
...
def sandbox_profiles(katalog, params):
    budget = get_named(budget_profiles(katalog, params), 'dynamisches-resilienzbudget')
    runtime = dict(budget['base_config'])

    return [
        {
            'name': 'offene-ausfuehrung',
            'label': 'Offene Ausfuehrung',
        ...
        {
            'name': 'werkzeug-zellen',
            'label': 'Werkzeug-Zellen',
        ...
        {
            'name': 'foederierte-sandbox-zellen',
            'label': 'Foederierte Sandbox-Zellen',
            'selected_tools': ['memory', 'analysis', 'execution', 'bridge', 'shield'],
            'multiplier': 1.0,
            'base_config': {
                **runtime,
        ...
        {
            'name': 'maximal-abgeschirmte-zellen',
            'label': 'Maximal abgeschirmte Zellen',
            'selected_tools': ['memory', 'analysis', 'execution', 'bridge', 'shield'],
            'multiplier': 1.02,
            'base_config': {
                **runtime,
...
def sandbox_metrics(config, result, params, profile):
    ...
    cell_depth_score = normalize(profile['cell_depth'], 1.0, 5.0)
    boundary_score = normalize(profile['boundary_strength'], 1.0, 5.0)
    relay_score = normalize(profile['relay_depth'], 1.0, 4.0)

    containment_strength = clamp01(
        mittelwert(
            [
                boundary_score,
                1.0 if config.get('enable_fault_isolation', False) else 0.0,
                1.0 - workflow.get('cluster_compromise_mean', 0.0),
                profile['bias']['containment'],
            ]
        )
    )
    routing_integrity = clamp01(
        mittelwert(
            [
                ...
                relay_score,
                traceability,
                profile['bias']['routing'],
            ]
        )
    )
    knowledge_partition = clamp01(
        mittelwert(
            [
                ...
                cell_depth_score,
                profile['bias']['knowledge'],
            ]
        )
    )
    sandbox_safety = clamp01(
        mittelwert(
            [
...
def run_context(seed, params, profile, kontext):
    ...
    sandbox_config = apply_tools(profile['base_config'], profile['selected_tools'], tool_catalog(), profile['multiplier'])
    ...
    result['sandbox_metrics'] = sandbox_metrics(config, result, params, profile)
    return result


def context_score(kontext_name, result, agent_count):
    metrics = result['sandbox_metrics']
    ...
    if kontext_name == 'startup':
        return (
            ...
            + 0.16 * metrics['knowledge_partition']
            + 0.12 * metrics['routing_integrity']
    ...
    if kontext_name == 'operations':
        return (
            ...
            + 0.14 * metrics['routing_integrity']
            + 0.10 * metrics['knowledge_partition']
            - 0.10 * metrics['sandbox_overhead']
        )
...
def summarize_runs(runs, profile, context_list, agent_count):
    ...
    for kontext in context_list:
        ...
        for metric_name in metrics:
            metrics[metric_name][name] = mittelwert([run[name]['sandbox_metrics'][metric_name] for run in runs])
...
def main():
    ...
    profiles = sandbox_profiles(katalog, params)
    context_list = kontexte()
    base_seed = int(os.getenv('KKI_SEED', str(DEFAULT_SEED)))

    print('=' * 84)
    print('KKI SANDBOX-ZELLEN-STUDIE')
    print('=' * 84)
    print('Vergleicht Zell- und Sandbox-Strukturen fuer operative Baugruppen,')
    print('damit Werkzeuge, Wissen und Aufsicht kontrolliert gekoppelt bleiben.\n')
    ...
    baseline = next(item for item in summaries if item['name'] == 'offene-ausfuehrung')

    print('\nBeste Sandbox-Zellstruktur:', best['label'])
    print(
    ...
    axes[0, 0].set_title('Kombinierter Sandbox-Score')
    ...
    axes[0, 1].bar(
        x + width / 2,
        [item['sandbox_safety']['stress'] for item in summaries],
        width,
        label='Sicherheit',
        color='#59a14f',
    )
    ...
    axes[0, 2].bar(
        x - width / 2,
        [item['routing_integrity']['operations'] for item in summaries],
        width,
        label='Routing',
        color='#76b7b2',
    )
    axes[0, 2].bar(
        x + width / 2,
        [item['knowledge_partition']['operations'] for item in summaries],
        width,
        label='Wissenszellen',
        color='#f28e2b',
    )
    axes[0, 2].set_title('Routing und Wissen')
    ...
    axes[1, 0].bar(
        x + width / 2,
        [item['sandbox_overhead']['operations'] for item in summaries],
        width,
        label='Overhead',
        color='#bab0ab',
    )
    ...
    fig.tight_layout()
    output = save_and_maybe_show(plt, 'kki_sandbox_zellen.png')
