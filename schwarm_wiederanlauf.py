"""
KKI Wiederanlauf-Studie
=======================
Vergleicht Wiederanlauf, Resynchronisation und Rekonfiguration nach Zell- und
Cluster-Ausfaellen unter Angriff und Quarantaene.
"""
...
def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_RESTART_REPETITIONS', '2'))
        ...
        repetitions = int(os.getenv('KKI_RESTART_REPETITIONS', '3'))
    ...
    return {
        ...
        'agent_count': int(os.getenv('KKI_RESTART_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
...
def profile(params):
    ...
    common = {
        **basis,
        ...
        'resource_budget': float(os.getenv('KKI_RESTART_RESOURCE_BUDGET', '0.55')),
    ...
    return [
        ...
        {
            'name': 'emergent-failure',
            'label': 'Skills mit Ausfall',
            **common,
        ...
        {
            'name': 'shielded-failure',
            'label': 'Quarantaene mit Ausfall',
            **common,
        ...
        },
        {
            ...
            **common,
            ...
            'resync_strength': float(os.getenv('KKI_RESYNC_STRENGTH', '0.24')),
        },
        {
            'name': 'reconfigured-restart',
            'label': 'Rekonfigurierter Wiederanlauf',
            **common,
            'enable_fault_isolation': True,
            ...
            'failure_fraction': max(0.12, float(os.getenv('KKI_FAILURE_FRACTION', '0.22')) - 0.04),
            'sentinel_share': 0.18,
            'analyzer_share': 0.22,
    ...
    return {
        ...
        'combined_score': mittelwert(
            [
                ...
                + 0.12 * pol['workflow_metrics'].get('resource_efficiency', 0.0)
                + 0.08 * pol['workflow_metrics'].get('trust_shield_mean', 0.0)
                + 0.14 * pol['workflow_metrics'].get('sync_strength_mean', 0.0)
                + 0.18
                * pol['workflow_metrics'].get('recovery_events_total', 0.0)
                / max(1, pol['workflow_metrics'].get('failure_events_total', 0))
                + 0.05 * pol['workflow_metrics'].get('reconfiguration_switch_total', 0.0) / max(1, pol['config'].get('agent_count', 1))
                - 0.06 * pol['workflow_metrics'].get('failed_agents_mean', 0.0) / max(1, pol['config'].get('agent_count', 1))
        ...
        'failed_agents_mean': mittelwert([run['workflow_metrics'].get('failed_agents_mean', 0.0) for run in polarization_runs]),
        ...
        'reconfigurations': mittelwert([run['workflow_metrics'].get('reconfiguration_switch_total', 0.0) for run in polarization_runs]),
    }
...
def main():
    ...
    print('=' * 84)
    print('KKI WIEDERANLAUF-STUDIE')
    print('=' * 84)
    print('Untersucht, ob Schwarmzellen nach Ausfaellen wieder anlaufen, sich resynchronisieren')
    print('und ihre Rollen bzw. Zellen adaptiv neu konfigurieren.\n')
    ...
    print(
        ...
        f"Resync-Niveau {best['sync_strength']:.2f}, "
    ...
    failed = [item['failed_agents_mean'] for item in summaries]
    sync = [item['sync_strength'] for item in summaries]
    ...
    ax3.bar(labels, failed, color=colors)
    ax3.set_title('Ø ausgefallene Agenten')
    ax3.tick_params(axis='x', rotation=18)

    ax4.bar(labels, sync, color=colors, label='Resync-Niveau')
    ax4.plot(labels, restart, color='#222222', marker='o', linewidth=2, label='Restart-Ereignisse')
    ax4.set_title('Wiederanlauf und Resynchronisation')
    ax4.tick_params(axis='x', rotation=18)
    ax4.legend()

    fig.tight_layout()
    save_and_maybe_show(plt, 'kki_wiederanlauf.png', dpi=150)
