"""
KKI Zustandstransfer-Studie
===========================
Vergleicht Zustandstransfer- und Wiederanlaufketten fuer laufende Baugruppen und
misst, welche Kopplung von Wissensbus, Rolloutfenstern und Restart-Logik Wissen,
Freigaben und Recovery ueber Zellwechsel hinweg konsistent haelt.
"""
...
from schwarm_wiederanlauf import profile as restart_profiles
from schwarm_wissensbus import knowledge_bus_profiles


def studienparameter():
    if is_test_mode():
        repetitions = int(os.getenv('KKI_STATE_TRANSFER_REPETITIONS', '2'))
        ...
        repetitions = int(os.getenv('KKI_STATE_TRANSFER_REPETITIONS', '3'))
    ...
    return {
        ...
        'agent_count': int(os.getenv('KKI_STATE_TRANSFER_AGENT_COUNT', str(DEFAULT_AGENT_COUNT))),
        'degree': int(os.getenv('KKI_NETWORK_DEGREE', str(DEFAULT_CONNECTIONS_PER_AGENT))),
        'stress_strength': float(os.getenv('KKI_STATE_TRANSFER_STRESS_STRENGTH', '0.48')),
        'stress_sources': int(os.getenv('KKI_STATE_TRANSFER_STRESS_SOURCES', '8')),
        'failure_duration': int(os.getenv('KKI_FAILURE_DURATION', '8')),
        'failure_fraction': float(os.getenv('KKI_STATE_TRANSFER_FAILURE_FRACTION', '0.18')),
    ...
    return [
        {
            ...
            'scenario': 'consensus',
            'startup_mode': True,
            'required_domains': ('memory', 'handoff', 'trace'),
        },
        {
            'name': 'migration',
            'label': 'Migration',
            'scenario': 'consensus',
            'migration_mode': True,
            'required_domains': ('memory', 'execution', 'coordination'),
        },
        {
            'name': 'stress',
            'label': 'Stress',
            ...
            'required_domains': ('memory', 'security', 'approval', 'trace'),
        },
        {
            'name': 'recovery',
            'label': 'Recovery',
            ...
            'required_domains': ('memory', 'execution', 'handoff', 'security'),
        },
    ]


def state_transfer_profiles(katalog, params):
    rollout = get_named(rollout_profiles(katalog, params), 'foederierter-rollout')
    knowledge = get_named(knowledge_bus_profiles(katalog, params), 'resilienter-wissensbus')
    restart = {item['name']: item for item in restart_profiles(params)}
    reconfigured = restart['reconfigured-restart']
    resync = restart['restart-resync']
    ...
    return [
        {
            'name': 'zustandsschnappschuss',
            'label': 'Zustandsschnappschuss',
        ...
        {
            'name': 'checkpoint-kette',
            'label': 'Checkpoint-Kette',
        ...
        {
            'name': 'foederierter-zustandstransfer',
            'label': 'Foederierter Zustandstransfer',
        ...
        {
            'name': 'restart-foederation',
            'label': 'Restart-Foederation',
            'selected_tools': ['memory', 'analysis', 'execution', 'bridge', 'shield'],
            'multiplier': 0.99,
            'base_config': {
                **runtime,
        ...
        {
            'name': 'resiliente-wiederanlaufkette',
            'label': 'Resiliente Wiederanlaufkette',
            'selected_tools': ['memory', 'analysis', 'execution', 'bridge', 'shield'],
            'multiplier': 1.0,
            'base_config': {
                **runtime,
...
def transfer_metrics(config, result, params, profile, kontext):
    ...
    transfer_depth_score = normalize(profile['transfer_depth'], 1.0, 5.0)
    handoff_score = normalize(profile['handoff_layers'], 1.0, 4.0)
    checkpoint_score = normalize(profile['checkpoint_layers'], 1.0, 4.0)
    restart_score = normalize(profile['restart_depth'], 1.0, 5.0)
    ...
    migration_stability = clamp01(
        mittelwert(
            [
                ...
                profile['bias']['migration'],
            ]
        )
    )
    recovery_chain = clamp01(
        mittelwert(
            [
                recoveries,
                1.0 - failed_share,
...
def run_context(seed, params, profile, kontext):
    ...
    transfer_config = apply_tools(profile['base_config'], profile['selected_tools'], tool_catalog(), profile['multiplier'])
    config.update(transfer_config)
    ...
    elif kontext.get('migration_mode'):
        config['enable_prompt_injection'] = False
    ...
    result['transfer_metrics'] = transfer_metrics(config, result, params, profile, kontext)
    return result
...
def summarize_runs(runs, profile, context_list, agent_count):
    ...
    for kontext in context_list:
        ...
        for metric_name in metrics:
            metrics[metric_name][name] = mittelwert([run[name]['transfer_metrics'][metric_name] for run in runs])
...
def main():
    ...
    profiles = state_transfer_profiles(katalog, params)
    context_list = kontexte()
    base_seed = int(os.getenv('KKI_SEED', str(DEFAULT_SEED)))

    print('=' * 84)
    print('KKI ZUSTANDSTRANSFER-STUDIE')
    print('=' * 84)
    print('Vergleicht Zustandstransfer und Wiederanlaufketten fuer laufende Baugruppen,')
    print('damit Wissen, Auditpfade und Recovery ueber Zellwechsel hinweg kontrolliert bleiben.\n')
    ...
    baseline = next(item for item in summaries if item['name'] == 'zustandsschnappschuss')

    print('\nBester Zustandstransfer:', best['label'])
    print(
    ...
    axes[0, 0].set_title('Kombinierter Transfer-Score')
    ...
    axes[0, 1].bar(
        x + width / 2,
        [item['continuity_memory']['migration'] for item in summaries],
        width,
        label='Kontinuitaet',
        color='#59a14f',
    )
    ...
    axes[0, 2].bar(
        x - width / 2,
        [item['audit_transfer']['stress'] for item in summaries],
        width,
        label='Audit-Transfer',
        color='#e15759',
    )
    axes[0, 2].bar(
        x + width / 2,
    ...
    axes[1, 0].bar(
        x - width / 2,
        [item['migration_stability']['migration'] for item in summaries],
        width,
        label='Migration',
        color='#76b7b2',
    )
    axes[1, 0].bar(
        x + width / 2,
        [item['transfer_overhead']['migration'] for item in summaries],
        width,
        label='Transfer-Overhead',
        color='#bab0ab',
    )
    axes[1, 0].set_title('Migration gegen Overhead')
    axes[1, 0].set_xticks(x)
    axes[1, 0].set_xticklabels(labels, rotation=18)
    axes[1, 0].legend()
    axes[1, 0].grid(True, axis='y', alpha=0.3)

    axes[1, 1].bar(x, [item['context_scores']['migration'] for item in summaries], color=colors)
    axes[1, 1].set_title('Migrations-Kontextscore')
    ...
    fig.tight_layout()
    output = save_and_maybe_show(plt, 'kki_zustandstransfer.png')
